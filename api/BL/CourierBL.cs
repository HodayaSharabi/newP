...
namespace BL
{
    public class CourierBL
    {
        ...
        public static MapTOCurios GetCourierDetailMaps(CourierDTO courier)
       {
            ...
            //MTC.LatLng = sortLatLon(MTC.LatLng, current);
            return MTC;
        ...
        public static List<LanLng> sortLatLon(List<LanLng> ListLatLon , LanLng beginPoint)
        {
            ...
            while (ListLatLon.Count!=0)
            {
                ...
                {
                    if (distans > PackageBL.GetDistance(currentPoint.Lat.Value, currentPoint.Lng.Value, item.Lat.Value, item.Lng.Value))
                    {
                        distans = PackageBL.GetDistance(currentPoint.Lat.Value, currentPoint.Lng.Value, item.Lat.Value, item.Lng.Value);
                        nextPoint = item;
                    }
                    ListLatLon.Remove(nextPoint);
                    mewList.Add(nextPoint);
